...
<cfinvoke component="#cffm#" method="init">
	<!--- includeDir = You can and probably should hard code this... by default, the directory is a 
		...
		--->
	<cfinvokeargument name="includeDir" value="#expandPath("../../uploads/ad_assets")#">
	<!--- includeDirWeb = web path to the directory specified above. 
		FOR RAM DRIVE:  use "/" (CF9+)
	--->
	<cfinvokeargument name="includeDirWeb" value="http://www.ncsanj.com/uploads/ad_assets">
	...
	// images or something.
	--->
	<cfinvokeargument name="allowedExtensions" value="jpg,gif,png,txt,html,htm,swf">
...
<cfelseif action eq "showDirectoryListing">
	...
	<cfif not cffm.readOnly>
		...
		<cfif cffm.allowCreateDirectory>
			 <td align="center" width="40"><a href="javascript:void(0);" onclick="showCreateDirectoryForm();"><img class="cffmIcon" src="#cffm.iconPath#/newfolder.png" width="32" height="32" alt="#cffm.resourcekit['buttonText.t8']#" title="#cffm.resourcekit['buttonText.t8']#">#cffm.resourcekit['buttonText.t8']#</a></div></td>
		</cfif>
	</cfif>
	</tr>
	</table>
...
<thead>
	...
	<cfoutput query="variables.sortedDirList"><cfsilent>
		...
		<cfif AllowImageEdit AND ((variables.imageComponent eq "imageobject_cfimage" or cffm.includeDir does not contain "ram://") and not cffm.readOnly and variables.editableImage)>
