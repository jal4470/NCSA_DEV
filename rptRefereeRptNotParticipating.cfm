...
<H1 class="pageheading">NCSA - Referee Reporting of players Not Participating</H1>
...
<cfif isDefined("FORM.GO") OR isDefined("FORM.GOSINGLE") >
	...
	<cfquery name="getResults" datasource="#application.dsn#">
		Select * from (
			...
			g.game_datetime, r.homeNoParticipateCnt as not_participating_cnt, mdf.match_day_form_id
			...
			where r.homeNoParticipateCnt > 0
		UNION ALL
			...
			g.game_datetime, r.visitorNoParticipateCnt as not_participating_cnt, mdf.match_day_form_id
			from tbl_referee_rpt_header r
			inner join v_games_all g
			on r.game_id=g.game_id
			left join tbl_match_day_form mdf
			on g.game_id=mdf.game_id and g.visitor=mdf.team_id
			where r.visitorNoParticipateCnt > 0
		) t
...
<FORM name="Games" action="rptRefereeRptNotParticipating.cfm"  method="post">
...
<div id="sortLinks" style="font-size:10px; margin:0 0 10px;">
	...
	<table id="resultTable" cellspacing="0" cellpadding="3" border="0" width="100%" class="table1">
		...
		<tr>
			...
			<th>Flight</th>
			<th>Date</th>
			<th>Team Not Participating</th>
			<th>Number Not Paticipating</th>
		</TR>
		</thead>
		<tbody>
