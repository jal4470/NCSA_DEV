<cfcomponent>
	...
	<cffunction
		...
		<cfset LOCAL.Custom_ColumnNames = [] />
		...
		<cfloop
			index="LOCAL.Custom_ColumnName"
			list="#ARGUMENTS.Fields#"
			delimiters=",">
	 
			<!--- Store the current column name. --->
			<cfset ArrayAppend(
				LOCAL.Custom_ColumnNames,
				Trim( LOCAL.Custom_ColumnName )
				) />
		...
		<cfset LOCAL.CustomColumnCount = ArrayLen( LOCAL.Custom_ColumnNames ) />
		...
		<cfif ARGUMENTS.CreateHeaderRow>
			...
			<cfloop
				index="LOCAL.CustomColumnIndex"
				from="1"
				to="#LOCAL.CustomColumnCount#"
				step="1">
	 
				<!--- Add the field name to the row data. --->
				<cfset LOCAL.RowData[ LOCAL.CustomColumnIndex ] = """#LOCAL.Custom_ColumnNames[ LOCAL.CustomColumnIndex ]#""" />
