...
<html>
	...
	<head>
		...
		<link type="text/css" rel="stylesheet" href="../ReportBuilder/assets/theme/jquery-ui-1.7.2.custom.css" />
...
<cfhtmlhead text="<title>Message Manager Step 2</title>">
<CFINCLUDE TEMPLATE="header_nltr.cfm"> 
	
		<cfoutput>
		<form method="post" action="Step2_action.cfm" onclick="return submitFormAction();">
...
<!--- 			<tr>
			...
			<cfif getCriteria.recordcount GT 0>
				<cfloop query="getCriteria">
					...
					<cfif len(trim(getCriteria.allow_edits)) and not getCriteria.allow_edits>
					...
					<tr class="criteriaRow">
						<td>
							...
							<!--- column select --->
							<select name="view_col_id" <cfoutput>#disabled#</cfoutput>>
								...
								<cfoutput query="getColumns" group="view_col_group_name">
									<optgroup label="#view_col_group_name#">
										<cfoutput>
											<option value="#view_col_id#" xtype="#xtype#" allow_lookup_flag="#allow_lookup_flag#" <cfif getCriteria.view_col_id EQ view_col_id>selected="selected"</cfif>>#left(view_col_display_label,maxRowChars)#</option>
										</cfoutput>
									</optgroup>
								</cfoutput>
						...
						<td>
							<!--- operand select --->
							<select name="operand_id" <cfoutput>#disabled#</cfoutput>>
						...
						<td>
							<!--- value input --->
							<cfoutput>
								<input type="text" name="value" size="40" value="#htmleditformat(getCriteria.value)#" <cfoutput>#readonly#</cfoutput>><cfif readonly neq "readonly"><img class="lookupTrigger" src="assets/images/magnifier.gif" style="vertical-align:top; margin-left:3px; width:20px; cursor:pointer;"> <a href="javascript:void(0);" class="removeCriteriaRow">Remove</a></cfif>
							</cfoutput>
						</td>
					</tr>
				</cfloop>
