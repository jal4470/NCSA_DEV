...
$.extend(Keypad.prototype, {
	...
	APPLY: AP, //Key marker for apply button
	...
	_generateHTML: function(inst) {
		...
		for (var i = 0; i < layout.length; i++) {
			...
			for (var j = 0; j < keys.length; j++) {
				...
				html += (keys[j] == this.SPACE ? '<div class="keypad-space"></div>' :
					...
					(keys[j] == this.APPLY ? ' keypad-apply' :
					...
					(keys[j] == this.APPLY ? this._get(inst, 'applyText') :
		...
		html.find('button').mousedown(function() { $(this).addClass('keypad-key-down'); }).
			...
			filter('.keypad-apply').click(function() {  }).end().
			...
			not('.keypad-clear').not('.keypad-back').not('.keypad-close').not('.keypad-apply').not('.keypad-shift').
