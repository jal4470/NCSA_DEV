...
<H1 class="pageheading">NCSA - Delayed Report of Score per Coach Login</H1>
...
<cfif isDefined("FORM.GO") OR isDefined("FORM.GOSINGLE") >
	...
	select fta.*, dbo.getTeamName(fta.team_id) as team_name, dbo.f_get_division(fta.team_id) as division, datediff(n,fta.game_datetime, fta.logcreatedate) as delay 
		from (
		select gl.game_id, g.game_datetime, gl.logcreatedate, tc.team_id,
		g.field
		from 
		(
			select game_id, min(logcreatedate) as logcreatedate, createdfrompage, logcreatedby, home_club_id, visitor_club_id from tbl_game_log group by game_id, createdfrompage, logcreatedby, home_club_id, visitor_club_id
		) gl
		inner join xref_game_team gt
		on gl.game_id=gt.game_id
		inner join
		(
			select team_id, contactidhead as coach_id
			from tbl_team
			<cfif Session.MenuRoleID EQ 28 OR Session.MenuRoleID EQ 27 OR Session.MenuRoleID EQ 26 OR Session.MenuRoleID EQ 25>
			where club_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#SESSION.USER.CLUBID#">
			</cfif>
			UNION ALL
			select team_id, contactidasst as coach_id
			from tbl_team
			<cfif Session.MenuRoleID EQ 28 OR Session.MenuRoleID EQ 27 OR Session.MenuRoleID EQ 26 OR Session.MenuRoleID EQ 25>
			where club_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#SESSION.USER.CLUBID#">
			</cfif>
		) tc
		on gt.team_id=tc.team_id and gl.logcreatedby=tc.coach_id
		...
		<cfelse>
			...
			<cfif flight NEQ "">
				AND dbo.f_get_division(tc.team_id) in (#listQualify(flight,"'")#)
			</cfif>
			<cfif delay NEQ "">
...
<FORM name="Games" action="rptDelayedScoreReport.cfm"  method="post">
