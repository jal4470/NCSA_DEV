...
<H1 class="pageheading">NCSA - Game Day Documents not submitted by game time</H1>
...
<cfif isDefined("FORM.GO") OR isDefined("FORM.GOSINGLE") >
	...
	<cfif swContinue>
		<cfquery name="qNCByGametime" datasource="#SESSION.DSN#">
			
			select g.Game_id, 
				DIVISION, game_datetime, HOME_TEAMNAME, VISITOR_TEAMNAME, 
...
<CFIF IsDefined("qNCByGametime")>
	...
	<cfloop query="qNCByGametime">
		<tr>
			...
			<td><a href="ViewGameDayDocs.cfm?game_id=#game_id#" target="_tab">View</a></td>
			<td>#trim(replace(missing_doc,',',' ','ALL'))#</td>
			<td>#refName#</td>
