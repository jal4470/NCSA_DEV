...
<cfif team_id NEQ "" AND NOT isdefined("game_id")>
	...
	<cfquery datasource="#application.dsn#" name="qGetGames">
		...
		dbo.f_get_choice_responses(69,81) a --game choice
		inner join dbo.f_get_choice_responses(70,81) b --team choice
		on a.response_id=b.response_id
		where b.value=<cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#team_id#">
		UNION
		select a.response_id, a.value as game_id, b.value as team_id from 
		dbo.f_get_choice_responses(69,82) a --game choice
		inner join dbo.f_get_choice_responses(70,82) b --team choice
		on a.response_id=b.response_id
		where b.value=<cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#team_id#">
		) r
...
<cfif not isdefined("game_id")>
	<cfif team_id EQ "">
	<FORM action="refEvaluation.cfm" method="post">
		...
		<table class="table1" cellpadding="3" cellspacing="0" border="0">
			...
			<cfloop query="qGetGames">
				<tr>
					...
					<td style="text-align:center;"><cfif response_id NEQ ""><a href="viewRefEvaluation.cfm?response_id=#response_id#">View</a></cfif></td>
				</tr>
			</cfloop>
			</tbody>
		</TABLE>
...
<cfelse>
	...
	<cfif getGame.asst1name NEQ "">
		<cfmodule template="forms/renderForm.cfm"
			...
			return_page="#Application.sitevars.homehttp#/refEvaluation.cfm?team_id=#team_id#">
	<cfelse>
		<cfmodule template="forms/renderForm.cfm"
			user_form_id="3" 
			label_vars="#label_vars#"
