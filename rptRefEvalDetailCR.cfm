<!--- 
	FileName:	rptRefEvalDetailCR.cfm
...
<H1 class="pageheading">NCSA - Detailed Ref Evaluation CR Specific</H1>
...
<cfif isDefined("FORM.GO") OR isDefined("FORM.GOSINGLE") >
	...
	<cfquery name="getResults" datasource="#application.dsn#" debug="yes">
		...
		dbo.getTeamName(fta.op_team_id) as opponent,
		dbo.f_get_contact_fullname(fta.ref_id) as ref_name from
		(
		select
		g.value as game_id, 
		q1.value as started_by_other,
		q2.value as contacted_by_ref,
		q3.value as time_loc_changed,
		q4.value as use_linesman,
		q5.value as instruct_linesman,
		q6.value as eval_about,
		gg.field, 
		gg.game_datetime, 
		t.value as team_id, 
		gt.team_id as op_team_id,
		r.value as ref_id
		from 
		(
			select response_id, value from dbo.f_get_choice_responses(69,81)
			UNION ALL
			select response_Id, value from dbo.f_get_choice_responses(69,82)
		) g
		left join dbo.f_get_qg_responses(56,25) q1
		on g.response_id=q1.response_id
		left join dbo.f_get_qg_responses(22,11) q2
		on g.response_id=q2.response_id
		left join dbo.f_get_qg_responses(23,11) q3
		on g.response_id=q3.response_id
		left join dbo.f_get_qg_responses(28,11) q4
		on g.response_id=q4.response_id
		left join dbo.f_get_qg_responses(29,11) q5
		on g.response_id=q5.response_id
		left join dbo.f_get_qg_responses(27,13) q6
		on g.response_id=q6.response_id
		inner join dbo.f_get_choice_responses(70,null) t
		on g.response_id=t.response_id
		inner join dbo.f_get_choice_responses(90,null) r
		...
		<cfelse>
			...
			<cfif isdefined("form.ref") AND form.ref NEQ "">
				AND r.value = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.ref#">
			</cfif>
			...
			<cfif isdefined("form.q5") AND form.q5 NEQ "">
				AND q5.value = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.q5#">
			</cfif>
			<cfif isdefined("form.q6") AND form.q6 NEQ "">
				AND q6.value = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.q6#">
			</cfif>
		</cfif>
			group by
		g.value, 
...
<FORM name="Games" action="rptRefEvalDetailCR.cfm"  method="post">
	...
	<tr>
		<td>
			...
			<cfquery name="getChoices" datasource="#application.dsn#">
				select choice_id, name
				from tbl_choice
				where question_group_id=13
			</cfquery>
			<b>Evaluation About</b>
			<select name="q6">
				<option value="">Select All</option>
				<cfloop query="getChoices">
...
<div id="sortLinks" style="font-size:10px; margin:0 0 10px;">
	...
	<table id="resultTable" cellspacing="0" cellpadding="3" border="0" width="100%" class="table1">
		...
		<tr>
			...
			<th>Started by Other</th>
			<th>Evaluation About</th>
			<th>Contacted by Ref</th>
			<th>Location Changed</th>
			<th>Use Club Linesman</th>
			<th>Instruct Club Linesman</th>
			<th>Referee</th>
		</TR>
		</thead>
		<tbody>
	<cfif getResults.recordCount GT 0>
		<cfloop query="getResults">
			<tr>
				...
				<td>#team_name#</td>
				<td>#started_by_other#</td>
				<td>#eval_about#</td>
				<td>#contacted_by_ref#</td>
				<td>#time_loc_changed#</td>
				<td>#use_linesman#</td>
				<td>#instruct_linesman#</td>
				<td>#ref_name#</td>
			</tr>
		</cfloop>
	<cfelse>
