...
<H1 class="pageheading">NCSA - Players Playing Up - Data List Report</H1>
...
<cfif isDefined("FORM.GO") OR isDefined("FORM.GOSINGLE") >
	<cfquery name="getResults" datasource="#application.dsn#" >
		select p.game_id, p.playLevel, p.game_datetime, p.play_up_team_name, p.name, p.pass_number, p.mdf_team_name, p.mdf_division
		from v_play_up p
		<cfif ListFind(clubRoles, Session.MenuRoleID) OR Clubselected GT 0>
		inner join tbl_team t on (p.play_up_team_id = t.team_id)
		</cfif>
		...
		<cfelse>
			...
			<cfif flight NEQ "">
				AND p.play_up_division in (#listQualify(flight,"'")#)
			</cfif>
		</cfif>
...
<FORM name="Games" action="rptPlayingUp.cfm"  method="post">
